...
class VideoConverterApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setAcceptDrops(True)  # Разрешаем drag-n-drop
        self.load_language_data()
        self.load_config()
        self.setWindowTitle(self.trans("title"))
        self.setGeometry(300, 300, 400, 540)
        ...
        self.drop_area = QLabel("Put it here")
        ...
        """)
        self.drop_area.setAcceptDrops(False)  # отключаем drag для самой QLabel, т.к. обрабатывается на уровне окна
    ...
    def load_config(self):
        try:
            with open("config.json", "r", encoding="utf-8") as f:
                config = json.load(f)
                self.current_language = config.get("language", "ru")
    ...
    def convert_video(self):
        ...
        ffmpeg_path = os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), "ffmpeg.exe")
